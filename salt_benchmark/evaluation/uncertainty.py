...
def AUROC(confidence: Sequence[float], ground_truth: Sequence[int | bool], sort: bool = True) -> float:
    """Compute rank-sum AUROC with average ranks for tied confidence scores."""
    scores = np.asarray(confidence, dtype=float)
    labels = np.asarray(ground_truth, dtype=float)
...
def auroc_score(confidence: Sequence[float], ground_truth: Sequence[int | bool], sort: bool = True) -> float | None:
    ...
    score = AUROC(confidence, ground_truth, sort=sort)
    if math.isnan(score):
        return None
    return score
